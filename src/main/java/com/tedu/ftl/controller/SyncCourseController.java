...
public class SyncCourseController {
	...
	public ResponseJSON resourceSync(Model model, HttpServletRequest request) {
		...
		for (CourseDo courseDo : list) {
			...
			List<Map<String, Object>> courseList = getParam("experiment/QryCourse", map);
			if (courseList.isEmpty()) {
				...
				log.info(map);
				save("experiment/InsertCourse", course);
				log.info(i);
			} else {
				course = new HashMap<String, Object>();
				...
				save("experiment/UpdateCourse", course);
				log.info(i);
			}
		}
